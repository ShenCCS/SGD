...
def dataloader():
    ...
    X_train = np.expand_dims(np.load(MRI_nii_folder_path + config["train_img_p3"]), axis=-1)[0:100]
    y_train = np.expand_dims(np.load(MRI_nii_folder_path + config["train_msk_p3"]), axis=-1)[0:100]
...
def dice_coef(targets, inputs, smooth=1e-6):
    y_true_f = K.flatten(targets)
    y_pred_f = K.flatten(inputs)
    intersection = K.sum(y_true_f * y_pred_f)
    loss = 1-((2. * intersection) / (K.sum(y_true_f) + K.sum(y_pred_f)))
    return loss
